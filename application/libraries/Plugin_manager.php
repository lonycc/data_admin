...
class Plugin_manager {
    ...
    public function __construct()
    {
        ...
        if ($this->app->settings->item('plugin_dev_mode')) {
            $this->app->load->model('cache_model');
            $this->app->cache_model->update_plugin_cache();
        }
        $this->initialize();
    }
    ...
    {
        //加载插件
        if ($this->app->platform->cache_exists(APPPATH . 'settings/plugins.php')) {
            include(APPPATH . 'settings/plugins.php');
...
abstract class DiliCMS_Plugin_Controller
{
	...
	public function __construct($name)
	{
		...
		$this->path = BASEPATH . '../plugins/' . $this->name . '/';
