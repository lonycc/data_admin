...
class Form
{
	...
	{
		$return = '<select  class="form-control" style="width:150px;" name="' . $field['name'] . '" id="' . $field['name'] . '">'.
	...
	private function _radio($field, $default, $place_holder='')
	{
		...
		$count = 1;
	      foreach ($field['values'] as $key=>$v) {
			$return .= '<input id="rad_' . $field['name'] . '_' . $count . '" name="' . $field['name'] . '" type="radio" value="' .
			           $key . '" ' . ($default == $key ? 'checked="checked"' : '') . ' />' . $v;
			$count ++;
		}
		return $return;
	}
	...
	private function _checkbox($field, $default, $place_holder='')
	{
		...
		if (is_array($field['values'])) {
			...
			$count = 1;
			foreach ($field['values'] as $key => $v) {
				$return .= 	'<input id="chk_' . $field['name'] . '_' . $count . '" name="' . $field['name'] . '[]" type="checkbox" value="' .
				            $key . '" ' . (in_array($key, $default) ? 'checked="checked"' : '') . ' />' . $v;
				$count ++;
			}
		} else {
			$return .= 	'<input id="chk_' . $field['name'] . '" name="' . $field['name'] . '" type="checkbox" value="1" ' .
			            ($default == 1 ? 'checked="checked"' : '') . ' />' . $field['values'];
		}
	...
	private function _wysiwyg($field, $default, $place_holder = '', $allow_upload = FALSE, $basic = FALSE)
	{
		...
		return '<textarea class="form-control" name="' . $field['name'] . '" id="' . $field['name'] . '" data-editor-width="'. $default_width .'" data-editor-height="'. $default_height .'"' .
	...
	{
		return '<input class="form-control" style="width:185px;" type="text" name="' . $field['name'] . '" id="' .
		       $field['name'] . '" value="' . $default . '" onFocus="WdatePicker({isShowClear:true,readOnly:true,dateFmt:\'yyyy-MM-dd HH:mm\'})"/>';
	}
	...
	private function _colorpicker($field, $default)
	{
		...
		return '<input class="field_colorpicker form-control" name="' . $field['name'] . '" id="' . $field['name'] .
	...
	private function _linked_menu($field , $default)
	{
		...
		$html .= '&nbsp;<a href="javascript:void();" onclick="linked_menu_insert(\'linked_menu_' . $options[0] . '\',\'' . $field['name'] . '\',' . $options[3] .
			     ');"  class="">添加</a>';
		...
		$html .= '</ul></div>';
		$html .= '<script>$(".linked_menu_' . $options[0] . '").ld({ajaxOptions : {"url" : "' .
