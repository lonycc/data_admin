...
class Acl
{
            ...
            if ($v['sub_menus']) {
                ...
                foreach ($v['sub_menus'] as $j) {
                    ...
                    if ($this->_current_menu ==  2) {
                        ...
                        plugin_url($key, $j['class_name'], $j['method_name']) . '"><span class="glyphicon glyphicon-star">' . $j['menu_name'] . '</a></li>';
                        continue;
                    }
                    echo '<li class="' . (isset($j['current']) ? 'selected' : '') .'"><a href="' .
                    backend_url($j['class_name'] . '/' . $j['method_name'], $extra) . '"><span class="glyphicon glyphicon-star">' . $j['menu_name'] . '</a></li>';
                }
                echo	 '</ul></li>';
            }
        }
     ...
     * @return  void
     */
    private function _filter_normal_menus($class_name, $method_name, $default_uri = 'index/index', $current_menu = 0, $folder = '')
    {//0
     ...
     * 过滤cms菜单
     */
    private function _filter_cms_menus($class_name, $method_name)
    {//3
