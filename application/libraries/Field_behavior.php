...
class Field_behavior {
    ...
    private function _load_extra_field($type) {
        if (!in_array($type, array_keys($this->_extra_fields))) {
            ...
            if (file_exists(APPPATH . '../fields/' . $extra_class . '.php')) {
                include (APPPATH . '../fields/' . $extra_class . '.php');
    ...
    public function on_info($data, $oldname = '') {
        if ($this->_is_extra($data['type'])) {
            ...
            switch ($data['type']) {
                ...
                case 'int':
                    $field = array('type' => 'INT', 'constraint' => $data['length'] ? $data['length'] : 10, 'default' => 0);
                break;
                case 'float':
                    $field = array('type' => 'FLOAT', 'constraint' => $data['length'] ? $data['length'] : 10, 'default' => 0);
                break;
                case 'input':
                case 'select':
                case 'radio':
    ...
    public function on_list($field, $value) {
        if ($this->_is_extra($field['type'])) {
            ...
            switch ($field['type']) {
                    /*
                    case 'thumbnail':
                    echo isset($field['values'][$value->$field['name']]) ? '也不知道是什么': '暂无缩略图';
                    */
                case 'radio':
                case 'select':
