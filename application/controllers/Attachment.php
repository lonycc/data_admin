...
class Attachment extends MY_Controller {
    ...
    public function _upload_post() {
        ...
        if ($session) {
            ...
            $now = now();
            if (!$userdata OR $now - $session->last_activity > $this->config->item('sess_expiration') OR $userdata['uid'] != $this->input->post('uid', TRUE)) {
                $status = "fail";
            } else {
                if (!$_FILES['Filedata']['error']) {
                    $data['folder'] = date('Y/m', $now);
                    $target_path = BASEPATH . '../' . setting('attachment_dir') . '/' . $data['folder'];
                    if ($status != 'fail') {
        ...
        if ($uid = $this->session->uid) {
            if (isset($_FILES[$field]) AND !$_FILES[$field]['error']) {
                ...
                if ($is_valid) {
                    ...
                    $target_path = BASEPATH . '../' . setting('attachment_dir') . '/' . $upload['folder'];
