...
class Category_content extends MY_Controller
{
    /**
     * 构造函数
     *
    ...
    public function _view_post()
    {
        ...
        $this->_check_permit();
        if ( ! $this->platform->cache_exists(APPPATH . 'settings/category/cate_' . $model . '.php'))
        {
    ...
    public function _save_post()
    {
        ...
        if ( $this->form_validation->run() == FALSE ) {
            ...
            $attachment = $this->input->post('uploadedfile', TRUE)?$this->input->post('uploadedfile', TRUE):'0';
            ...
            if ($id) {
                ...
                $this->user_model->writeLog($this->session->user, "修改分类模型[{$modeldata['description']}]id为[{$id}]的数据");
                ...
                $this->user_model->writeLog($this->session->user, "添加数据到分类模型[{$modeldata['description']}]");
    ...
    public function _del_post()
    {
        ...
        $model = $this->input->get('model', TRUE);
        $model_data = $this->db->select('id,level,auto_update')->where('name', $model)->get($this->db->dbprefix('cate_models'))->row();
        $model_id = $model_data->id;
        $model_level = $model_data->level;
        if ($ids) {
            if ( ! is_array($ids)) {
                $ids = array($ids);
            }
            ...
            foreach ($ids as $v) {
                ...
                $where_string .= ($model_level == 1)?" OR classid=".$v:" OR path Like '" . $_category->path . ',{' . $v . "}%'";
            }
            $result = $this->db->select('classid')->where($where_string)->get($this->db->dbprefix('u_c_') . $model)->result();
            foreach ($result as $v) {
            ...
            $idss = implode(', ', $ids);
            $this->user_model->writeLog($this->session->user, "删除了分类模型[{$model}]的数据[{$idss}]");
        }
        $this->_message('删除操作成功完成!', '');
    }
