...
class Db_model extends CI_Model {
    ...
    public function get_folder_status() {
        if (!file_exists($this->_backup_path)) {
            return '备份文件夹不存在!请先在app目录下建立backup文件夹';
    ...
    public function get_unoptimized_tables() {
        $return = array('tables' => array(), 'total_size' => 0);
        $table_type = intval($this->db->version()) > 4.1 ? 'Engine' : 'Type';
        $tables = $this->db->query('SHOW TABLE STATUS')->result_array();
        foreach ($tables as $table) {
            if ($table['Data_free'] > 0 && $table[$table_type] == 'MyISAM') {
                $return['tables'][] = array($table['Name'], $table[$table_type], $table['Rows'], $table['Data_length'], $table['Index_length'], $table['Data_free']);
    ...
    public function optimize() {
        //return $this->dbutil->optimize_database();  //ci3.0不支持优化功能
        $return = true;
        $tables = $this->get_unoptimized_tables();
        foreach ($tables['tables'] as $table) {
            if ($table[1] == 'InnoDB') {
                $return = $this->db->query('analyze table ' . $table[0]);
            } else {
                $return = $this->db->query('optimize table ' . $table[0]);
            }
        }
        return $return;
    }
    ...
    public function export($filename = 'db', $tables = array(), $table_id = 0, $is_compress = FALSE, $volume_size = 2048, $start = 0, $is_extend_insert = FALSE, $volume) {
        ...
        if (trim($sql)) {
            ...
            return array('volume' => $volume, 'filename' => rawurlencode($filename), 'is_compress' => $is_compress, 'volume_size' => $volume_size, 'start' => $this->_start_row, 'table_id' => rawurlencode($table_id), 'is_extend_insert' => $is_extend_insert);
        }
        //all done!
        return TRUE;
    }
