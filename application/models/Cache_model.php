...
class Cache_model extends CI_Model {
    ...
    private function _create_folder($folder = '') {
        if (!file_exists(APPPATH . 'settings/' . $folder)) @mkdir(APPPATH . 'settings/' . $folder);
    }
    // ------------------------------------------------------------------------
    ...
    public function update_model_cache($target = '') {
        ...
        foreach ($models as $model) {
            $model['fields'] = array();
            $model['fields_org'] = $this->db->where('model', $model['id'])->order_by('`order`', 'ASC')->get($this->db->dbprefix('model_fields'))->result_array();
            ...
            $this->platform->cache_write(APPPATH . 'settings/model/' . $model['name'] . '.php', array_to_cache("setting['models']['" . $model['name'] . "']", $model));
        }
    }
    // ------------------------------------------------------------------------
    ...
    public function update_category_cache($target = '') {
        ...
        foreach ($models as $model) {
            $model['fields'] = array();
            $model['fields_org'] = $this->db->where('model', $model['id'])->order_by('`order`', 'ASC')->get($this->db->dbprefix('cate_fields'))->result_array();
            $model['listable'] = array();
            $model['searchable'] = array();
            foreach ($model['fields_org'] as $key => & $v) {
                if ($v['listable'] == 1) {
            ...
            $this->platform->cache_write(APPPATH . 'settings/category/cate_' . $model['name'] . '.php', array_to_cache("setting['cate_models']['" . $model['name'] . "']", $model));
            $category = array();
            $categories = $this->category_model->get_category($model['name']);
            foreach ($categories as $c) {
                $category[$c['classid']] = $c;
            }
            $this->platform->cache_write(APPPATH . 'settings/category/data_' . $model['name'] . '.php', array_to_cache("setting['category']['" . $model['name'] . "']", $category));
    ...
    public function update_menu_cache() {
        ...
        $level_1_menus = $this->db->select('menu_id, class_name, method_name, menu_name')->where('menu_level', 0)->where('menu_parent', 0)->get($table_menu)->result_array();
        foreach ($level_1_menus as & $i) {
            $level_2_menus = $this->db->select('menu_id, class_name, method_name, menu_name')->where('menu_level', 1)->where('menu_parent', $i['menu_id'])->get($table_menu)->result_array();
            foreach ($level_2_menus as & $j) {
                if ($j['class_name'] == 'content') {
                    $level_3_menus = $this->db->select(" 'content' AS class_name, 'view' AS 'method_name', name AS extra, description AS menu_name", FALSE)->get($this->db->dbprefix('models'))->result_array();
                } else if ($j['class_name'] == 'category_content') {
                    $level_3_menus = $this->db->select(" 'category_content' AS class_name, 'view' AS 'method_name', name AS extra, description AS menu_name", FALSE)->get($this->db->dbprefix('cate_models'))->result_array();
                } else {
                    $level_3_menus = $this->db->select('menu_id, class_name, method_name, menu_name')->where('menu_level', 2)->where('menu_parent', $j['menu_id'])->get($table_menu)->result_array();
                }
                $j['sub_menus'] = $level_3_menus;
            }
            $i['sub_menus'] = $level_2_menus;
        }
        $this->platform->cache_write(APPPATH . 'settings/menus.php', array_to_cache("setting['menus']", $level_1_menus));
    }
    // ------------------------------------------------------------------------
    ...
    public function update_role_cache($target = '') {
        ...
        foreach ($roles as & $role) {
            ...
            $rights = $this->db->select('right_class, right_method, right_detail')->where_in('right_id', $role['rights'])->get($this->db->dbprefix('rights'))->result();
            $role['rights'] = array();
            ...
            $this->platform->cache_write(APPPATH . 'settings/acl/role_' . $role['id'] . '.php', array_to_cache("setting['current_role']", $role));
        }
    }
    // ------------------------------------------------------------------------
    ...
    public function update_site_cache() {
        ...
        $this->platform->cache_write(APPPATH . 'settings/site.php', array_to_cache("setting", $data));
    }
    // ------------------------------------------------------------------------
    ...
    public function update_backend_cache() {
        ...
        $this->platform->cache_write(APPPATH . 'settings/backend.php', array_to_cache("setting", $data));
    }
    // ------------------------------------------------------------------------
    ...
    public function update_plugin_cache() {
        ...
        $plugins = $this->db->select('name, access')->where('active', '1')->get($this->db->dbprefix('plugins'))->result_array();
        if ($plugins) {
            foreach ($plugins as $key => $plugin) {
                if (!isset($cached_plugins[$plugin['name']])) {
                    $cached_plugins[$plugin['name']] = array('classmap' => array(), 'menus' => array(), 'access' => $plugin['access']);
                }
                $this_plugin_path = APPPATH . 'plugins/' . $plugin['name'] . '/';
        ...
        $this->platform->cache_write(APPPATH . 'settings/plugins.php', array_to_cache("setting['plugins']", $cached_plugins));
    }
    /**
     * 更新字段类型缓存
     *
    ...
    public function update_fieldtypes_cache() {
        ...
        $extra_path = APPPATH . 'settings/fields/';
        ...
        $this->platform->cache_write(APPPATH . 'settings/fieldtypes.php', array_to_cache("setting", $cached_fieldtypes));
    }
    /* 删除缓存文件 */
    public function update_data_cache() {
        foreach (glob(APPPATH . "cache/*_*") as $file) {
            unlink($file);
        }
    }
}
