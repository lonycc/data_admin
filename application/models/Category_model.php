...
class Category_model extends CI_Model {
    ...
    public function get_child_num($classid, $model) {
        $data = $this->db->select("COUNT(*) as children")->where("parentid", $classid)->get($this->db->dbprefix('u_c_') . $model)->row_array();
     ...
     * @return  object
     */
    public function get_category_models($limit, $offset = 0) {
        return $this->db->limit($limit, $offset)->get($this->db->dbprefix('cate_models'))->result();
    }
    public function get_category_num() {
        return $this->db->count_all_results($this->db->dbprefix('cate_models'));
    }
    // ------------------------------------------------------------------------
    ...
    public function edit_category_model($target_model, $data) {
        if ($this->db->where('id', $target_model->id)->update($this->db->dbprefix('cate_models'), $data)) {
            ...
            if ($old_table_name != $data['name']) {
                ...
                $this->platform->cache_delete(APPPATH . 'settings/category/cate_' . $old_table_name . '.php');
                $this->platform->cache_delete(APPPATH . 'settings/category/data_' . $old_table_name . '.php');
            }
            return TRUE;
        }
        return FALSE;
    ...
    public function del_category_model($model) {
        $this->load->dbforge();
        //删除表
        $this->dbforge->drop_table('u_c_' . $model->name, true);
        //删除字段
        $this->db->where('model', $model->id)->delete($this->db->dbprefix('cate_fields'));
        //删除附件
        $attachments = $this->db->select('name, folder, type')->where('model', $model->id)->where('from', 1)->get($this->db->dbprefix('attachments'))->result();
        ...
        //清除缓存文件
        $this->platform->cache_delete(APPPATH . 'settings/category/cate_' . $model->name . '.php');
        $this->platform->cache_delete(APPPATH . 'settings/category/data_' . $model->name . '.php');
    }
    // ------------------------------------------------------------------------
